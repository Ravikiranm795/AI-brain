...
- **`blastRadiusSummary`**: full totals (`total`, `highConfidence`/`lowConfidence`, per-file counts) even when the returned list is capped at `maxCallers` (default 40) — so a truncated answer can never be mistaken for a complete one.
- **`risk`**: `"covered"`, `"untested"`, `"incomplete"`, or `"unresolved"` — see below.
- **`confidence`** (per blast-radius entry): `high` when the edge was resolved through the receiver's type, a repo-unique name, or a language server; `low` when it matched only by a shared method name. Low-confidence edges never underwrite a `"covered"` verdict — a `stream()` call on a `List` is not a call into your `StreamService`, and treating it as one previously produced a 1,700-entry "blast radius" for a class that three files referenced.

**Checking a class or interface**: a call almost never attributes to the class itself (`this.svc.getRoleTypes()` points at the `getRoleTypes` method symbol, not `svc`'s class) — see §11. `brain check`/`brain expand` handle this automatically: for a `class`-kind symbol, results are unioned across every member method the parser found, plus anywhere the class is directly constructed (`new Foo()`). A `classAggregation: { memberCount, unresolved }` field is included whenever this aggregation kicked in. When `memberCount` is 0 — most commonly a TS `interface`, whose members parse as type signatures rather than symbols — there was nothing to walk from, so `unresolved: true` and `risk: "unresolved"` are set instead of a plain empty result. **Treat `unresolved` as "couldn't determine", never as "safe to change"** — this is the one case where an empty `blastRadius`/`testsCovering` does *not* mean "looked, found nothing".

**Index gaps — when brain admits it can't answer.** A file can be in your repo but not fully in the index: it blew the size cap (§2), or its AST parse failed and it was indexed as plain text. If any such file *mentions* the symbol you're checking, `risk` becomes `"incomplete"` and `indexGaps` names the files to grep. This exists because the alternative is far worse: a confident-looking empty blast radius that simply couldn't see half the repo. **`"incomplete"` and `"unresolved"` both mean "unknown" — only `"covered"` and `"untested"` are actual findings.** The `brain build` output reports the same thing up front as an `Index health:` line.
...
**`brain_list` defaults to just the current repo**, not every repo ever indexed on the machine — it returns a lightweight summary (`rootDir`/`builtAt`/`fileCount`/`stats`, never the full per-file manifest) plus a count of how many other repos exist. Pass `all: true` (with optional `limit`/`offset`) to page through every indexed repo instead. This is a deliberate fix over the older behavior of always returning every repo's raw manifest, which could blow past a client's response size limit on a machine with a lot of indexed history.
...
Each result's `score` is a [reciprocal-rank-fusion](https://plg.uwaterloo.ca/~gvcormac/cormacksigir09-rrf.pdf) score combining four independent signals:
...
4. An **exact-identifier** boost: if the query text contains a word that's the exact (case-insensitive) name of a real symbol — e.g. a query that already says "AuthService" — that symbol is forced to the top of the results, ahead of the RRF fusion of the other three signals entirely. This exists specifically because semantic search alone can rank an unrelated-but-similar-sounding hit (e.g. a "remote login" settings page) above the actual symbol you named — a plain `grep` for a known identifier doesn't have that failure mode, so this signal gives `brain search` the same reliability for the common case of "I already know its name."
...
- `confidence` buckets it into something actionable: `"high"` means at least two of the four signals agreed on this result (an exact-identifier match always counts as agreement); `"medium"`/`"low"` mean only one did, based on how high it ranked there.
- **If you already know the exact name of what you're looking for, put it in the query verbatim** (e.g. `brain search "AuthService login method"` rather than `brain search "how does auth work"`) — signal 4 above only fires on an exact match, and is the most reliable of the four when it applies.
...
- **Call-edge resolution is receiver-type-aware, but not a type checker.** A call resolves in tiers: `typed` (the receiver's declared type owns a method by that name — from fields, parameters including Angular/Nest constructor injection, and locals), `unique` (exactly one symbol in the repo has that name), `external` (the receiver's type isn't a class this repo defines, e.g. `List`/`String` — recorded with no target, since the callee lives outside the index), then `heuristic` (name-only, flagged and excluded from any confident claim). There's no cross-file type inference, no generics, no inheritance chain: a call through an inherited method or an un-annotated variable falls back to the name tiers. `--precise` (§7) layers real language-server resolution on top for TS/JS.
- **A class/interface itself almost never has direct call edges** — the parser never gives a class body its own scope (see `parser.js`), so a call made through `this.method()` attributes to the *method* symbol, not the class. `brain check`/`brain expand` compensate for this automatically (§4) by aggregating across a class's member methods, plus capturing `new Foo(...)` as a direct edge to `Foo`. This still can't see "which specific instance" a call runs through (no type-flow analysis), and a TS interface's members (`method_signature` nodes) aren't captured as symbols at all, so an interface always comes back `unresolved` rather than a real (possibly-empty) answer.
...
- **First `brain build` seems slow / needs internet** — that's the one-time ~90MB embedding model download. Subsequent runs are fully offline. On a large repo (thousands of files), a first/forced build's `onProgress` log now reports parsing and embedding as two separate phases with running percentages (`...embedded 1280/4000 symbols (32%)`) rather than going quiet for minutes — embedding runs as a handful of large batched passes over every new/changed symbol (not one pass per file), which is also meaningfully faster on a from-scratch build of a large repo than earlier versions of this tool.
